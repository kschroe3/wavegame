function scr_lobby_ui() {
    ...
    for (var c = 0; c < 4; c++) {
        ...
        if (!my_player.ready && scr_draw_button(btn_x, base_y + 40, 70, 40, "")) {
            my_player.class = c;
            scr_net_send_update();
        }
    ...
    for (var col = 0; col < 4; col++) {
        ...
        var text_col = selected ? global.available_colors[col] : c_black;
        draw_set_color(taken ? c_dkgray : (selected ? c_lime : global.available_colors[col]));
        if (!my_player.ready && !taken && scr_draw_button(btn_x, base_y + 100, 70, 40, "")) {
            my_player.color = col;
            scr_net_send_update();
        }
    ...
    // Ready button
    var ready_text = my_player.ready ? "UNREADY" : "READY?";
    var ready_col = my_player.ready ? c_orange : c_yellow;
