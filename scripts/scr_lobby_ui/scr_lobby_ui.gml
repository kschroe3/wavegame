function scr_lobby_ui() {
    ...
    var base_y = 180 + global.my_id * 90;
    ...
    draw_text(x_center - 200, base_y + 50, "Class:");
    for (var c = 0; c < 4; c++) {
        var btn_x = x_center - 150 + c * 80;
        var selected = (my_player.class == c);
        draw_set_color(selected ? c_lime : c_gray);
        if (scr_draw_button(btn_x, base_y + 70, 70, 40, global.class_names[c])) {
            my_player.class = c;
            scr_net_send_update();
        }
    }
    ...
    // Color select
    draw_set_color(c_white);
    draw_text(x_center - 200, base_y + 130, "Color:");
    for (var col = 0; col < 4; col++) {
        ...
        var btn_x = x_center - 150 + col * 80;
        draw_set_color(taken ? c_dkgray : global.available_colors[col]);
        if (!taken && scr_draw_button(btn_x, base_y + 150, 70, 40, "")) {
            my_player.color = col;
            scr_net_send_update();
        }
        draw_set_color(c_white);
        draw_text(btn_x + 35, base_y + 170, "●");
    }
    ...
    if (scr_draw_button(x_center + 100, base_y + 100, 100, 50, ready_text)) {
