...
if (event_type == network_type_data) {
    ...
    if (global.is_host && cmd == 1) { // CMD_JOIN: Client sends passcode
        ...
        // Simple passcode check (later: make secure)
    ...
    } else if (!global.is_host && cmd == 0) { // CMD_ACCEPT: From host
        buffer_delete(buf);
    ...
    // Client connected — waiting for CMD_JOIN packet
    // Do nothing here — handled in network_type_data
